...
main() {
    ...
    echo
    echo -e "To test the service, run:"
    echo -e "docker-compose exec app grpcurl -plaintext -emit-defaults \\
    -d '{
    ...
    localhost:${GRPC_PORT} connector.v1.ConnectorService/CreateConnector | jq"
    echo
    echo -e "${YELLOW}Debugging Tips:${NC}"
    echo -e "1. View logs:"
    echo -e "   docker-compose logs -f app"
    echo
    echo -e "2. Access container shell:"
    echo -e "   docker-compose exec app bash"
    echo
    ...
    echo -e "\n${GREEN}Complete End-to-End Testing Flow:${NC}"

    echo -e "\n${YELLOW}Step 1: OAuth Flow${NC}"
    echo "a) Get OAuth URL and authorize the application:"
    echo "docker-compose exec app grpcurl -plaintext -emit-defaults \\
    -d '{
        \"redirect_uri\": \"https://localhost:${HTTP_PORT}/oauth/callback\"
    }' \\
    localhost:${GRPC_PORT} connector.v1.ConnectorService/GetOAuthV2URL | jq"
    echo "Open the URL in your browser and follow the instructions. The server will automatically handle the OAuth code exchange."

    echo -e "\n${YELLOW}Step 2: Connector Management${NC}"
    echo "a) Create a connector with the token received from the OAuth flow:"
    echo "docker-compose exec app grpcurl -plaintext -emit-defaults \\
    -d '{
        \"workspace_id\": \"7656730043137\",
        \"tenant_id\": \"440\",
    ...
    localhost:${GRPC_PORT} connector.v1.ConnectorService/GetConnector | jq"

    echo -e "\nd) Try getting non-existent connector (error case):"
    echo "docker-compose exec app grpcurl -plaintext -emit-defaults \\
    -d '{
    ...
    echo -e "\n${YELLOW}Step 3: Message Testing${NC}"
    echo "a) Send a test message using CLI:"
    echo "docker-compose exec app go run go-server/cmd/cli/main.go \
        --connector-id=\"<connector_id>\" \
        --message=\"Test message from connector service\""

    echo -e "\nb) Try sending with invalid connector ID (error case):"
    echo "docker-compose exec app go run go-server/cmd/cli/main.go \
        --connector-id=\"00000000-0000-0000-0000-000000000000\" \
        --message=\"This should fail\""

    echo -e "\n${YELLOW}Step 4: Cleanup Operations${NC}"
    echo "a) Delete the connector:"
    echo "docker-compose exec app grpcurl -plaintext -emit-defaults \\
    -d '{
        \"id\": \"<connector_id>\",
        \"workspace_id\": \"7656730043137\",
    ...
    localhost:${GRPC_PORT} connector.v1.ConnectorService/DeleteConnector | jq"

    echo -e "\nb) Verify deletion by trying to get the deleted connector (should fail):"
    echo "docker-compose exec app grpcurl -plaintext -emit-defaults \\
    -d '{
        \"id\": \"<connector_id>\"
    }' \\
    ...
    echo -e "\nc) Try deleting non-existent connector (error case):"
    echo "docker-compose exec app grpcurl -plaintext -emit-defaults \\
    -d '{
        \"id\": \"00000000-0000-0000-0000-000000000000\",
        \"workspace_id\": \"7656730043137\",
    ...
    echo -e "\n${YELLOW}Notes:${NC}"
    echo "1. The OAuth flow will automatically exchange the code and provide you with the token"
    echo "2. Replace <connector_id> with the ID received from CreateConnector"
    echo "3. Each step depends on the successful completion of previous steps"
    echo "4. Error cases are included to demonstrate proper error handling"
    echo "5. The cleanup step is crucial for removing test data"

    echo -e "\n${YELLOW}Expected Results:${NC}"
    echo "1. OAuth Flow: Should return valid URL and automatically handle code exchange"
    echo "2. Create Connector: Should succeed with valid data, fail with invalid"
    echo "3. Get Connector: Should return data for valid ID, error for invalid"
    echo "4. Send Message: Should succeed with valid connector, fail with invalid"
    echo "5. Delete Connector: Should succeed for existing connector, fail for non-existent"
